...
M.on_attach = function(client, bufnr)
	...
	keymap("n", "<leader>D", "<cmd>lua vim.diagnostic.open_float({ scope = 'line' })<CR>", opts) -- Line diagnostic (float)
	keymap("n", "<leader>d", "<cmd>lua vim.diagnostic.open_float()<CR>", opts) -- Cursor diagnostic
	keymap("n", "<leader>pd", "<cmd>lua vim.diagnostic.jump({count = -1, float = true})<CR>", opts) -- previous diagnostic
	keymap("n", "<leader>nd", "<cmd>lua vim.diagnostic.jump({count = 1, float = true})<CR>", opts) -- next diagnostic
	...
	keymap("n", "<leader>gr", "<cmd>FzfLua lsp_refernces<CR>", opts) --  Show all refernces to the symbol under the cursor
